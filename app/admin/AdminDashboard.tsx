...
const moduleTitles:Record<string,{eyebrow:string;title:string}> = {
  ...
  testimonials:{eyebrow:"CONTENT / TRUST",title:"Client testimonials"},
...
type Appointment={id:number;reference:string;name:string;phone:string;service:string;consultationMode:string;preferredDate:string;preferredTime:string;status:string;paymentStatus:string;amount:number;durationMinutes:number;meetingUrl:string;assignedTo:string};
...
type Enrollment={id:number;reference:string;studentName:string;email:string;phone:string;courseTitle:string;status:string;paymentStatus:string;progress:number;gatewayStatus?:string;gatewayOrderId?:string};
...
export default function AdminDashboard({displayName,module:initialModule="overview",allowedModules=["*"]}:{displayName:string;module?:string;allowedModules?:string[]}) {
  ...
  return <div className={`admin-shell admin-module-${module}`}>
    ...
    <main className="admin-main">
      ...
      <section className="admin-panel legacy-cms-pages" id="legacy-pages">
      ...
      <section className="admin-panel appointment-admin" id="appointments">
        ...
        <div className="appointment-table">
          ...
          {appointments.length===0?<p className="empty-row">No consultation requests yet.</p>:appointments.map(x=><article key={x.id}><div><b>{x.name}</b><small>{x.reference} · {x.phone}</small><span className={`payment-pill ${x.paymentStatus}`}>{x.paymentStatus} {x.amount?`· ${new Intl.NumberFormat("en-IN",{style:"currency",currency:"INR",maximumFractionDigits:0}).format(x.amount/100)}`:""}</span></div><div><b>{x.service}</b><small>{x.consultationMode} · {x.durationMinutes} min</small></div><div><b>{x.preferredDate}</b><small>{x.preferredTime}</small>{x.meetingUrl&&<a href={x.meetingUrl} target="_blank">Open meeting ↗</a>}</div><select value={x.status} onChange={e=>update("/api/appointments",x.id,e.target.value)}><option>payment-pending</option><option>pending</option><option>confirmed</option><option>completed</option><option>cancelled</option></select></article>)}
        </div>
      </section>

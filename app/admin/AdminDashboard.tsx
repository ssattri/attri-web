...
export default function AdminDashboard({displayName,module:initialModule="overview"}:{displayName:string;module?:string}) {
  ...
  return <div className={`admin-shell admin-module-${module}`}>
    ...
    <main className="admin-main">
      <header><div><p>{current.eyebrow}</p><h1>{module==="overview"?`Good morning, ${displayName.split(" ")[0]}.`:current.title}</h1><span className="admin-header-context">Signed in as {displayName} · secure session active</span></div><div><NotificationCenter compact/><Link href="/admin/reset-password">Account security</Link><Link href="/" target="_blank" rel="noreferrer">View website ↗</Link>{module==="products"?<Link className="admin-header-action" href="/admin/products/new">＋ Add product</Link>:module==="courses"?<Link className="admin-header-action" href="/admin/courses/new">＋ Add course</Link>:<Link className="admin-header-action" href="/admin?module=pages" onClick={event=>navigateModule("pages",event)}>＋ Quick create</Link>}</div></header>

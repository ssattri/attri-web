...
export default function AdminLoginForm({showActivation=false}:{showActivation?:boolean}) {
  ...
  return <form className="admin-login-form" onSubmit={submit}><label>Administrator email<input name="email" type="email" required autoComplete="username" placeholder="admin@example.com" /></label><label>Password<input name="password" type="password" required autoComplete="current-password" placeholder="Your secure password" /></label>{error && <div className="registration-error" role="alert">{error}</div>}<button disabled={busy}>{busy ? "Verifying…" : "Enter control centre →"}</button>{showActivation&&<a href="/admin/setup">First-time owner activation</a>}<a href="/">← Return to website</a></form>;
}

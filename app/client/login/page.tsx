...
export default async function ClientLogin(){const user=await getChatGPTUser();if(user)redirect("/account");const userReturn=encodeURIComponent("/client/onboarding?role=user"),consultantReturn=encodeURIComponent("/client/onboarding?role=consultant");return <main className="client-access-page">
 ...
 <section className="client-access-card role-access-card"><div><p>ACCOUNT REGISTRATION</p><h2>How will you use Attri?</h2><span>Select your account type. You will verify your identity with Google, email, or ChatGPT on the next step.</span></div>
  <a className="role-choice user-choice" href={`/signin-with-chatgpt?return_to=${userReturn}`}><strong>01</strong><span><b>I am a User</b><small>Buy courses, products, consultations, services and Vastu software subscriptions.</small><em>Register / continue as User →</em></span></a>
  <a className="role-choice consultant-choice" href={`/signin-with-chatgpt?return_to=${consultantReturn}`}><strong>02</strong><span><b>I am a Consultant</b><small>Use the cloud Vastu Compass, manage client projects, analyses and reports.</small><em>Register / continue as Consultant →</em></span></a>

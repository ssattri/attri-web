...
export async function POST(request:Request){
  ...
  const found=await db.prepare(`SELECT id,name,CASE WHEN special_price>0 AND (special_from='' OR date('now')>=special_from) AND (special_to='' OR date('now')<=special_to) THEN special_price ELSE price END AS price,stock,gst_rate AS gstRate,weight,dimensions,item_type AS itemType FROM products WHERE id IN (${ids.map(()=>"?").join(",")}) AND status='active'`).bind(...ids).all<{id:number;name:string;price:number;stock:number;gstRate:number;weight:string;dimensions:string;itemType:string}>();
  const map=new Map(found.results.map(x=>[x.id,x]));let subtotal=0;let items;
  try{items=body.items.map(x=>{const product=map.get(Number(x.id)),quantity=Math.max(1,Math.min(10,Number(x.quantity)||1));if(!product||product.stock<quantity)throw new Error();subtotal+=product.price*quantity;return{id:product.id,name:product.name,price:product.price,quantity,gstRate:product.gstRate,weight:product.weight,dimensions:product.dimensions,itemType:product.itemType}})}catch{return Response.json({error:"One or more products are unavailable in the requested quantity."},{status:409})}

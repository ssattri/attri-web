...
import { calculateShipping } from "../../../../../platform-modules";
...
export async function POST(request:Request){
  ...
  const found=await db.prepare(`SELECT id,name,CASE WHEN special_price>0 AND (special_from='' OR date('now')>=special_from) AND (special_to='' OR date('now')<=special_to) THEN special_price ELSE price END AS price,stock,gst_rate AS gstRate,hsn_code AS hsnSac,weight,dimensions,item_type AS itemType FROM products WHERE id IN (${ids.map(()=>"?").join(",")}) AND status='active'`).bind(...ids).all<{id:number;name:string;price:number;stock:number;gstRate:number;hsnSac:string;weight:string;dimensions:string;itemType:string}>();
  const map=new Map(found.results.map(x=>[x.id,x]));let subtotal=0;let items;
  try{items=body.items.map(x=>{const product=map.get(Number(x.id)),quantity=Math.max(1,Math.min(10,Number(x.quantity)||1));if(!product||product.stock<quantity)throw new Error();subtotal+=product.price*quantity;return{id:product.id,name:product.name,price:product.price,quantity,gstRate:product.gstRate,hsnSac:product.hsnSac,weight:product.weight,dimensions:product.dimensions,itemType:product.itemType}})}catch{return Response.json({error:"One or more products are unavailable in the requested quantity."},{status:409})}
  const physical=items.filter(x=>x.itemType==="physical"),shipping=physical.length?await calculateShipping(physical,subtotal,body.state):{amount:0,rule:"Digital delivery"},shippingAmount=shipping.amount,total=subtotal+shippingAmount,reference=`ORD-${Date.now().toString(36).toUpperCase()}`;
  const inserted=await db.prepare("INSERT INTO orders(reference,customer_name,email,phone,address,city,state,pincode,gstin,items_json,subtotal,shipping_amount,total,payment_method,status,payment_status,admin_notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,'payment-pending','pending',?) RETURNING id").bind(reference,body.name.trim(),user.email.toLowerCase(),body.phone.trim(),body.address.trim(),body.city.trim(),body.state.trim(),body.pincode,String(body.gstin||"").trim().toUpperCase(),JSON.stringify(items),subtotal,shippingAmount,total,"razorpay",body.gstin?`GSTIN: ${body.gstin.trim().toUpperCase()}`:"").first<{id:number}>();
  ...
  return Response.json({keyId:e.RAZORPAY_KEY_ID,orderId,amount:total,currency:"INR",reference,shippingAmount,shippingRule:shipping.rule,prefill:{name:body.name,email:user.email,contact:body.phone}});
}

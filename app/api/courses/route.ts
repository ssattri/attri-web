...
export async function GET(request:Request){await init();const d=await db();const slug=new URL(request.url).searchParams.get("slug")?.trim().toLowerCase();const query=slug?"SELECT c.id,c.title,c.slug,c.category,c.level,c.mode,c.duration,c.description,c.price,c.lessons,c.image_url AS imageUrl,c.instructor,c.certificate,c.seats,c.meta_title AS metaTitle,c.meta_keywords AS metaKeywords,c.meta_description AS metaDescription,(SELECT COUNT(*) FROM enrollments e WHERE e.course_id=c.id AND e.status NOT IN ('cancelled')) AS enrolled FROM courses c WHERE c.status='published' AND lower(c.slug)=? ORDER BY c.id DESC":"SELECT c.id,c.title,c.slug,c.category,c.level,c.mode,c.duration,c.description,c.price,c.lessons,c.image_url AS imageUrl,c.instructor,c.certificate,c.seats,c.meta_title AS metaTitle,c.meta_keywords AS metaKeywords,c.meta_description AS metaDescription,(SELECT COUNT(*) FROM enrollments e WHERE e.course_id=c.id AND e.status NOT IN ('cancelled')) AS enrolled FROM courses c WHERE c.status='published' ORDER BY c.id DESC";const r=slug?await d.prepare(query).bind(slug).all():await d.prepare(query).all();return Response.json({courses:r.results})}

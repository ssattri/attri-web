...
export async function GET(){await init();const d=await db();const r=await d.prepare("SELECT id,title,slug,category,level,mode,duration,description,price,lessons FROM courses WHERE status='published' ORDER BY id").all();return Response.json({courses:r.results})}

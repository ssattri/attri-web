...
export async function GET(){
  ...
  const courses=await d.prepare("SELECT id,title,slug,category,level,mode,duration,description,price,lessons,status,image_url AS imageUrl,instructor,certificate,show_in_shop AS showInShop,created_at AS createdAt FROM courses ORDER BY id DESC").all();
...
export async function POST(request:Request){
  ...
  try{await d.prepare("INSERT INTO courses (title,slug,category,level,mode,duration,description,price,lessons,status,image_url,instructor,certificate,show_in_shop) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
    .bind(b.title.trim(),slugify(b.slug||b.title),b.category,b.level||"Beginner",b.mode||"Recorded",b.duration||"",b.description||"",price,lessons,b.status==="published"?"published":"draft",b.imageUrl||"",b.instructor||"Attri Academy Faculty",b.certificate==="yes"?1:0,b.showInShop==="yes"?1:0).run()}
...
export async function PATCH(request:Request){
  ...
  if(b.kind==="course"){
    ...
    if(b.title){
      ...
      try{await d.prepare("UPDATE courses SET title=?,slug=?,category=?,level=?,mode=?,duration=?,description=?,price=?,lessons=?,status=?,image_url=?,instructor=?,certificate=?,show_in_shop=? WHERE id=?")
        .bind(b.title.trim(),slugify(b.slug||b.title),b.category,b.level||"Beginner",b.mode||"Recorded",b.duration||"",b.description||"",price,lessons,b.status==="published"?"published":"draft",b.imageUrl||"",b.instructor||"Attri Academy Faculty",b.certificate==="yes"?1:0,b.showInShop==="yes"?1:0,b.id).run()}
      catch{return Response.json({error:"That course URL slug already exists."},{status:409})}
    }else{

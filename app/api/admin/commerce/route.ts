...
export async function PATCH(request:Request){
  ...
  if(body.kind==="product"){
    ...
    if(body.name){
      ...
      try{await database.prepare("UPDATE products SET name=?,slug=?,category=?,description=?,price=?,stock=?,status=?,image_url=?,item_type=?,delivery_mode=?,special_price=?,special_from=?,special_to=?,duration=?,classes=?,sort_order=?,meta_title=?,meta_keywords=?,meta_description=?,service_type=?,fulfillment_mode=? WHERE id=?")
        .bind(body.name.trim(),slugify(body.slug||body.name),body.category.trim(),body.description?.trim()??"",price,stock,body.status==="draft"?"draft":"active",body.imageUrl?.trim()??"",itemType,body.deliveryMode==="Offline"?"Offline":body.deliveryMode==="Online + Offline"?"Online + Offline":"Online",specialPrice,body.specialFrom||"",body.specialTo||"",itemType==="course"?body.duration||"":"",itemType==="course"?classes:0,sortOrder,body.metaTitle?.trim()||body.name.trim(),body.metaKeywords?.trim()||"",body.metaDescription?.trim()||body.description?.trim()||"",itemType==="service"&&(body.serviceType==="consultation"||body.serviceType==="virtual")?body.serviceType:"",itemType==="service"?body.fulfillmentMode||"Email":"",body.id).run()}

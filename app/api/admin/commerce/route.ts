...
async function ensureCategories(database:Awaited<ReturnType<typeof db>>){
  ...
  const columns=await database.prepare("PRAGMA table_info(product_categories)").all<{name:string}>();
  for(const [name,sql] of [["description","ALTER TABLE product_categories ADD COLUMN description TEXT NOT NULL DEFAULT ''"],["icon_url","ALTER TABLE product_categories ADD COLUMN icon_url TEXT NOT NULL DEFAULT ''"]])if(!columns.results.some(x=>x.name===name))await database.prepare(sql).run();
...
export async function PATCH(request:Request){
  if(!await allowed())return Response.json({error:"Unauthorized"},{status:401});
  const body=await request.json() as {id?:number;kind?:string;status?:string;paymentStatus?:string;trackingNumber?:string;adminNotes?:string;name?:string;slug?:string;category?:string;description?:string;iconUrl?:string;price?:string|number;stock?:string|number;imageUrl?:string;itemType?:string;deliveryMode?:string;specialPrice?:string|number;specialFrom?:string;specialTo?:string;duration?:string;classes?:string|number;sortOrder?:string|number;metaTitle?:string;metaKeywords?:string;metaDescription?:string;serviceType?:string;fulfillmentMode?:string;sku?:string;shortDescription?:string;material?:string;colour?:string;dimensions?:string;weight?:string;placement?:string;benefits?:string;usageInstructions?:string;careInstructions?:string;gstRate?:string|number;hsnCode?:string};
  if(!body.id)return Response.json({error:"A record is required."},{status:400});
  const database=await db();
  if(body.kind==="category"){await ensureCategories(database);if(!body.name?.trim())return Response.json({error:"Enter a category name."},{status:400});try{await database.prepare("UPDATE product_categories SET name=?,slug=?,description=?,icon_url=?,status=?,sort_order=? WHERE id=?").bind(body.name.trim(),slugify(body.name),body.description?.trim()||"",body.iconUrl?.trim()||"",body.status==="inactive"?"inactive":"active",Number(body.sortOrder||0),body.id).run();return Response.json({success:true})}catch{return Response.json({error:"That category already exists."},{status:409})}}
